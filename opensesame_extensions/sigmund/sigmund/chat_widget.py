...
class MultiLineInput(QPlainTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        # By default, QPlainTextEdit captures Enter/Return to insert a newline,
        # so we'll handle it in keyPressEvent below.

    def keyPressEvent(self, event):
        # Pressing Enter → send message (unless Shift is pressed).
        if event.key() == Qt.Key_Return and not (event.modifiers() & Qt.ShiftModifier):
            # Emit a signal so ChatWidget can handle the sending
...
class ChatWidget(BaseWidget):
    ...
    The Sigmund extension connects to user_message_sent signal
    ...
    def _init_ui(self):
        ...
        # Connect textChanged to enable/disable send based on input length
